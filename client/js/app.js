...
document.addEventListener('DOMContentLoaded', () => {
    // Note: initializeMockData() is no longer needed with API-based data manager
    ...
    // Handle initial route if no hash is present
    if (!window.location.hash) {
        window.location.hash = '/';
    } else {
        // Trigger route handler manually
        router.handleRoute();
    }
});
    ...
    lucide.createIcons();
        ...
        mainContent.innerHTML = `
            <div class="bg-red-50 border border-red-200 rounded-lg p-6">
                ...
                <p class="text-red-600">${error.message || 'Failed to load data from server'}</p>
                <p class="text-sm text-red-500 mt-2">Make sure the backend server is running at http://localhost:8000</p>
            </div>
        `;
    }
}
...
async function renderRecentActivity() {
    try {
        ...
        return '<p class="text-gray-500 text-sm">Failed to load recent activity</p>';
    }
}
